...
`DispatchIndirectCommand` (12 B) — declared on the C3 side in `gpu/compat/command.c3`
...
extern struct C3 declaration already exists (e.g. gpu/compat/command.c3); emits the GLSL
...
```text
gpu/compat/shader_abi.c3                          (library source, module gpu::compat)

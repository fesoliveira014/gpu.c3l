...
- [x] Split global Vulkan state into runtime-owned discovery and device-owned backend state across `gpu/internal/vk/backend.c3`, `gpu/internal/vk/device.c3`, `gpu/internal/vk/internal.c3`, and resource tables.
  - **Depends on:** 2.1.
...
- [x] Remove deferred core resource release from `gpu/internal/vk/deferred.c3` and public destruction paths, resolving issue #199 and the lifetime portion of #200.
...
- [ ] Implement only the approved compatibility descriptor model under `gpu/compat/` and private `gpu/internal/vk/` sources in `gpu::internal::vk`, reusing root allocations, spans, textures, views, samplers, and completion points.
  - **Depends on:** 8.2.
...
- [ ] Define and test a private feature and extension matrix in `gpu/internal/vk/` mapping approved semantics to promoted core or extension facilities without changing public requests or capability names.
  - **Depends on:** Gate D.

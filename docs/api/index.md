...
`DeviceCaps`, `TimestampCaps`, `SparseTextureCaps`, `MeshShaderCaps`,
...
`TextureViewCreateDesc`, `TextureIndexRange`, `TextureIndex`, `SamplerIndex`,
`SamplerDesc`,
...
`GpuSpan.unchecked_subspan`, `GpuSpan.checked_subspan`, `GpuSpan.allocation`,
`MappedGpuSpan.checked_subspan`, `TextureCompatibility.is_valid`.

Functions: `allocate_memory`, `allocate_mapped_memory`, `free_allocation`,
`get_allocation_info`, `get_allocation_span`, `get_span_mapping`,
...
`create_texture_view_at`, `update_texture_view`, `destroy_texture_view`,
`reserve_texture_indices`, `release_texture_indices`, `intern_sampler`,
...
`ComputePipelineDesc`, `GraphicsPipelineDesc`, `MeshPipelineDesc`,
`DrawMeshTasksIndirectCommand`, `RayTracingHitGroupKind`,
...
`cmd_draw_mesh_tasks_indirect_count`, `create_mesh_pipeline`,

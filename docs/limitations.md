...
These limits apply to `gpu::compat`. Strict `gpu` has no runtime operations yet.
...
|---|---|---|---|
| Texture descriptors in the heap | 4096 default, 65 536 max (`gpu/compat/descriptor_heap.c3:3`) | `texture_descriptor_capacity` | `DESCRIPTOR_HEAP_FULL` |
| Sampler descriptors | 256 default (`gpu/compat/descriptor_heap.c3:4`) | `sampler_descriptor_capacity` | `DESCRIPTOR_HEAP_FULL` |
| Live textures | 1024 default, 65 536 max (`gpu/compat/texture.c3:3`) | `texture_capacity` | `SLOT_TABLE_FULL` |
| Live buffers | 4096 (`gpu/compat/buffer.c3:3`) | — | `SLOT_TABLE_FULL` |
| Live pipelines / shaders | 256 each by default (`gpu/compat/pipeline.c3:3-4`) | `pipeline_capacity` for pipelines | `SLOT_TABLE_FULL` |
...
| Live semaphores | 256 (`gpu/compat/sync.c3:6`) | — | `SLOT_TABLE_FULL` |
| Live command records | 4096 (`gpu/compat/vk/command_state.c3:7`) | — | `SLOT_TABLE_FULL` |
| Swapchains | 8 (`gpu/compat/swapchain.c3:3`) | — | `SLOT_TABLE_FULL` |
| Color attachments per pass | Lesser of 8 (`gpu/compat/pipeline.c3:6`) and the selected device limit, reported by `DeviceCaps.max_color_attachments` | — | `INVALID_ARGUMENT` |
| Frame arena (per frame in flight) | 1 MiB (`gpu/compat/memory.c3:36`) | `frame_arena_size` | `ARENA_FULL` |
| Persistent arena | 64 MiB (`gpu/compat/memory.c3:37`) | `persistent_arena_size` | `ARENA_FULL` |
| Staging arena | 32 MiB default (`gpu/compat/memory.c3:38`) | `staging_arena_size` | `ARENA_FULL` |
| Readback arena | 8 MiB default (`gpu/compat/memory.c3:39`) | `readback_arena_size` | `ARENA_FULL` |

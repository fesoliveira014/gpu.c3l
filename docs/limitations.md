...
- **Packed descriptor ceilings are not guaranteed hardware capacities.** On the
  ...
  as storage images, so aggregate usage is `2 * texture_descriptor_capacity +
  sampler_descriptor_capacity`. `create_device` returns `INVALID_ARGUMENT`
  rather than clamping when any per-type, per-stage aggregate, or all-pools
  update-after-bind limit is exceeded.

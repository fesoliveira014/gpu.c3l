...
- Tiered threading: explicit caller-owned command allocators, thread-confined
  ...
  reclamation
- Command-buffer graphics state: attachment-only render-pass begin, an atomic
  begin-with-state convenience, and explicit complete/partial state that can be
  reused across compatible pass boundaries
